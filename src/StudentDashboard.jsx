...
export default function StudentDashboard({ students: studentList }) {
  ...
  const checkHasFailingScores = () => {
    ...
    return students.some((student) => {
      return (
        student.Chemistry < 30 || student.Physics < 30 || student.Math < 30
      );
    }); // Replace this line
  };
  ...
  const getAverageMathScore = () => {
    ...
    const totalMath = students.reduce((sum, student) => {
      return sum + student.Math;
    }, 0);
  ...
  const getAveragePhysicsScore = () => {
    ...
    const totalPhysics = students.reduce((sum, student) => {
      return sum + student.Physics;
    }, 0);
    ...
    <div style={styles.container}>
      ...
      <section style={styles.bannerSection}>
        {checkHasFailingScores() && (
          ...
          >
            ⚠️ <strong>Attention Needed:</strong> One or more students scored
            below 30 in a subject!
          </div>
        )}
        {checkIsHighPerformingClass() && (
          ...
          >
            🌟 <strong>High Performing Class:</strong> Every student currently
            holds an "A" or "B+" grade!
          </div>
        )}
      </section>
        ...
        <div style={styles.miniList}>
          {getTopThreeStudents().map((s, idx) => (
            <div key={s["Roll No."] || idx} style={styles.miniCard}>
              <strong>
                #{idx + 1} {s.Student_Names}
              </strong>{" "}
              (Roll: {s["Roll No."]}) - Grade: {s.Grade}
            </div>
          ))}
        </div>
      </section>
        ...
        <section style={styles.card}>
          ...
          {foundStudent && (
            <div style={styles.highlightBox}>
              <strong>Found:</strong> {foundStudent.Student_Names} | Grade:{" "}
              {foundStudent.Grade} | Phone: {foundStudent["Phone_No."]}
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
            <button onClick={handleFindLastAStudent}>
              Get Last 'A' Student
            </button>
            <button onClick={handleFindLastAtRiskIndex}>
              Index of Last At-Risk (Chem &lt; 40)
            </button>
          </div>

          {highlightedStudent && (
            <div style={styles.highlightBox}>
              <strong>Last 'A' Student:</strong>{" "}
              {highlightedStudent.Student_Names} (Roll No:{" "}
              {highlightedStudent["Roll No."]})
            </div>
          )}
        </section>
        ...
        <section style={styles.card}>
          ...
          <form onSubmit={handleAddStudent} style={styles.formGrid}>
            <input
              ...
              onChange={(e) =>
                setNewStudent({ ...newStudent, Student_Names: e.target.value })
              }
              required
            />
            <input
              ...
              onChange={(e) =>
                setNewStudent({
                  ...newStudent,
                  "Roll No.": parseInt(e.target.value, 10),
                })
              }
              required
            />
            <input
              ...
              onChange={(e) =>
                setNewStudent({
                  ...newStudent,
                  Math: parseInt(e.target.value, 10),
                })
              }
            />
            <input
              ...
              onChange={(e) =>
                setNewStudent({
                  ...newStudent,
                  Physics: parseInt(e.target.value, 10),
                })
              }
            />
            <input
              ...
              onChange={(e) =>
                setNewStudent({
                  ...newStudent,
                  Chemistry: parseInt(e.target.value, 10),
                })
              }
            />
            <select
        ...
        <div style={styles.grid}>
          {getFilteredStudents().map((student, index) => (
            <div key={student["Roll No."] || index} style={styles.studentCard}>
              ...
              <p>
                <strong>Roll No:</strong> {student["Roll No."]}
              </p>
              <p>
                <strong>Grade:</strong>{" "}
